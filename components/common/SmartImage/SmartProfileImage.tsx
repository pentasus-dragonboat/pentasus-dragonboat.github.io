// components/common/SmartImage/SmartProfileImage.tsx - Optimized with proper exports
import React, { useState } from 'react';
import type { TeamMember } from '@/lib/content/types';
import { getImagePath } from '@/lib/utils/image';
import { hasDiceBearMinimumData, getNameInitials, type DiceBearConfig } from '@/lib/utils/profile';

export interface SmartProfileImageProps {
  member: TeamMember;
  className?: string;
  alt?: string;
  loading?: 'lazy' | 'eager';
...
function generateDiceBearUrl(config: DiceBearConfig): string {
  ...
  if (config.eyesColor) params.set('eyesColor', config.eyesColor);
  ...
  if (hasDiceBearMinimumData(member.dicebear)) {
    ...
    return (
      <img
        ...
        onError={() => onImageTypeChange?.('initials')}
